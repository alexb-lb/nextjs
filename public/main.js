...
// domain Public Suffix List library end
...
var setLbCookies = ({ name, value = "", shareCookies = false }) => {
  const date = new Date();
  date.setTime(date.getTime() + 365 * 24 * 60 * 60 * 1000); // save cookie preferences for 1 year
  const expires = "; expires=" + date.toUTCString();
  const valueJson = value ? JSON.stringify(value) : "";
  const domainName = shareCookies ? "; domain=" + getLbMainDomain() : "";
...
!(function (t, e) {
  ...
  (t.unblock = function () {
    ...
    for (
      var a = document.querySelectorAll('script[type="'.concat(e, '"]')), s = 0;
      s < a.length;
      s++
    ) {
      var f = a[s];
...
// iframe blocking script start
window.lbIframeHandle = {
  blockedIframes: [], // Array<{ dataTag: string, src: string }>

  block: function (iframe) {
    const dataTag = Math.random().toString(36).substring(2, 5);

    lbIframeHandle.blockedIframes.push({
      dataTag,
      src: iframe?.src,
      style: iframe.style,
    });
  ...
  unblock: function (regexp = "") {
    document.querySelectorAll("iframe").forEach((iframe) => {
      const blockedItem = lbIframeHandle.blockedIframes?.find(
        (blocked) => blocked.dataTag === iframe.dataset.lbTag
      );
      if (!blockedItem) return;

      const shouldUnblock =
        regexp?.length > 0 ? blockedItem?.src?.match(regexp) : true;
      if (shouldUnblock) {
        lbIframeHandle.blockedIframes.filter(
          (blocked) => blocked.src === iframe.src
        );
  ...
  init: function () {
    // everything unblocked by default in case of preferece center selected
    if (window?.YETT_BLACKLIST && window?.YETT_BLACKLIST.length === 0) {
      return;
    }
      ...
      mutations.forEach(({ addedNodes }) => {
        addedNodes.forEach((node) => {
          if (node.nodeType === 1 && node.tagName === "IFRAME") {
            const src = node.src || "";
            const isInWhiteList = window.YETT_WHITELIST?.find((regexp) =>
              src?.match(regexp)
            );
            !isInWhiteList && lbIframeHandle.block(node);
          }
        });
      });
    });
...
window.lbIframeHandle?.init();
// iframe blocking script end
