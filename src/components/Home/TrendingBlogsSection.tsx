  ...
  return (
    <div className="flex overflow-hidden flex-col flex-1 shrink self-stretch pb-6 my-auto bg-white rounded-2xl basis-0 min-w-[240px]">
      <div className="flex overflow-hidden flex-col w-full rounded-xl">
        {thumbnail && (
          <Image
            ...
            height={100}
            width={100}
            className=" w-full aspect-[1.72] h-[280px] object-cover"
          />
        )}
      </div>
      ...
      <div className="flex gap-10 justify-between items-center px-6 pt-6 pb-2 md:py-6 w-full min-h-[80px] max-md:px-5">
        <div className="self-stretch px-[6.5px] py-[5px] my-auto text-sm font-medium tracking-tight leading-none text-[#7B808A] rounded bg-[#F5F7FA] ">
          {category?.title}
        </div>
        <div className="flex gap-2 justify-center items-center self-stretch my-auto para1 font-semibold text-[#545CF6] font-urbanist max-sm:absolute max-sm:bottom-[18px] max-sm:right-[18px] max-sm:text-[13px] cursor-pointer">
          <span
            className="self-stretch my-auto md:text-[16px] text-[20px]"
            onClick={() => route.push(`/resources/blogs/${slug}`)}
          >
            Read More
          </span>
          <img
...
const TrendingBlogsSection = ({
  ...
  useEffect(() => {
    const blogs = blogsData?.filter((blog: any) => {
      const tags = blog?.tags?.map((ele: any) => ele.title);
    ...
    >
      <div className="flex flex-col w-full max-md:max-w-full">
        ...
        <div className="hidden sm:flex flex-wrap gap-5 items-center mt-12 w-full max-md:mt-10 max-md:max-w-full">
          {sliceBlogs.map((blog: any, index: any) => (
