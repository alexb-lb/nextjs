...
const Footer = ({ sectionData, navigation = [] }: any) => {
  ...
  return (
    ...
    >
      <div className="flex overflow-hidden relative flex-col justify-center items-center self-stretch px-20 py-16 w-full min-h-[360px] max-md:px-5 max-md:max-w-full bg-[url('/images/contact/newsletter_bg.png')] bg-cover bg-no-repeat bg-center">
        <div className="flex relative flex-col items-center max-w-full w-[717px]">
          <div className="flex flex-col pr-px pl-2 w-full">
            <h2 className="lg:text-5xl text-[32px] leading-[40.32px] font-sora font-semibold gradient_heading capitalize text-center max-md:max-w-full max-md:text-4xl">
              ...
              {sectionData?.title || "Subscribe to Our Newsletter"}
            </h2>
          ...
          <form className="flex overflow-hidden flex-wrap gap-2.5 md:justify-center justify-between items-center py-2 pr-3 md:pl-6 pl-4 md:mt-8 mt-[40px] text-white rounded-[50px] max-md:pl-5  max-md:w-full border border-[#fff]">
            <input
              ...
              className="self-stretch my-auto md:text-md text-[18px] md:w-[300px] w-[126px] font-urbanist bg-transparent border-none"
              style={{ outline: "none" }}
              value={email}
      ...
      <div className="mt-12 w-full max-w-[1282px] max-md:mt-2 max-md:max-w-full px-[16px] min-xl:px-[20px] z-20">
            ...
            <div className="flex flex-col items-start w-full text-lg tracking-wide text-white max-md:mt-10">
              ...
              <div className="flex gap-3 self-stretch mt-10">
                ...
                <div className="flex-auto my-auto w-[247px]">
                  <Link href="mailto:info@lightbeam.ai">
          ...
          <div className="flex flex-col ml-5 lg:w-[74%] w-[65%] max-md:ml-0 max-md:w-full">
            <div className="flex w-full max-md:mt-10 max-md:max-w-full">
              <div className="flex flex-wrap flex-auto gap-10 md:gap-[72px] items-start max-md:mr-0 max-md:max-w-full">
                {navigation?.map((ele: any, idx: number) => (
                  <FooterColumn
                    key={idx}
                    title={ele?.name}
                    link={ele?.custom_path}
                    items={ele?.children?.map((item: any) => ({
                      title: item?.name,
                      link: item?.custom_path,
                    }))}
                  />
                ))}
                {/* <FooterColumn
      ...
      <div className="flex flex-wrap gap-5  justify-between mt-12 w-full max-w-screen-xl px-[16px]  max-md:mt-10 max-md:max-w-full min-xl:px-[20px]">
...
const FooterColumn: React.FC<FooterColumnProps> = ({ title, link, items }) => {
  return (
    <div className="flex flex-col whitespace-nowrap">
      ...
      <div className="flex flex-col items-start mt-8 text-base tracking-tight text-white/70 ">
        {items.map((item: any, index: number) => (
          <div
            key={index}
            className="flex flex-col justify-center mt-4 first:mt-0"
          >
            <div className="font-urbanist hover:text-white/90">
