...
import { whitePaperModel } from "@/utils/models/index";
...
const Page = ({ strapiData, navigation, caseStudy, GetInTouchData }) => {
  ...
  // whitePaper data from strapi
  const [whitePaper, setWhitePaper] = useState([]);
  useEffect(() => {
    (async function () {
      initializeStrapiClient();

      const response = await whitePaperModel.getDocuments({
        populate: {
          image: {
            populate: "*",
          },
      ...
      setWhitePaper(response.data.data);
    ...
    <>
      <div className=" bg-primary_white relative">
        <div className="relative">
          ...
          <SearchPapers strapiData={strapiData} whitePaper={whitePaper} />
