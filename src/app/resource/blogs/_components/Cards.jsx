...
const BlogCard = ({ image, title, description, category, slug }) => {
  const route = useRouter();
  return (
    <div className="flex h-full flex-col overflow-hidden  bg-white rounded-2xl  min-w-[240px]">
      <div className="flex flex-col overflow-hidden w-full rounded-xl">
        {image && (
          <Image
            ...
            src={image || null}
            alt={title}
            className="object-cover w-full aspect-[1.72]"
          />
        )}
      </div>
      ...
      <div className="flex gap-10 justify-between items-center px-6 py-6 w-full min-h-[80px] max-md:px-5">
        <div className="self-stretch px-[6.5px] py-[5px] my-auto text-sm font-medium tracking-tight leading-none text-[#7B808A] rounded bg-[#F5F7FA] w-[147px]">
          {category}
        </div>
