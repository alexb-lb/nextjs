...
const SearchPapers = ({ ebookData }) => {
  const [viewAll, setViewAll] = useState(false);
  const [search, setSearch] = useState("");
  const [books, setBooks] = useState();
  useEffect(() => setBooks(ebookData), [ebookData]);

  const [filterSelect, setFilterSelect] = useState(options[0]);

  function filterAlpha() {
    const sortedBlogs = [...books].sort((a, b) =>
      a.title.localeCompare(b.title)
    );
    setBooks(sortedBlogs);
  }

  function filterDateCreated() {
    const sortedBlogs = [...books].sort(
  ...
  return (
    <div>
      ...
      <div className="flex flex-nowrap max-md:px-4 md:flex-wrap md:gap-6 items-center justify-center md:justify-start md:w-[90%] md:mx-auto">
        ...
        <ContentPage
          contentList={books
            ...
            ?.filter((item, ind) =>
              viewAll ? ind < ebookData.length : ind < 9
            )}
          value={filterSelect}
        />
      </div>
