...
function TestimonialSection({ sectionData }) {
  const bg_img =
    (sectionData?.images?.data &&
      sectionData?.images?.data[0]?.attributes?.url) ||
    "/images/home/Quote_bg.png";
  return (
    <section
      // style={{ backgroundPositionY: `${value}`, paddingTop: `${pt}` }}
      className=" flex overflow-hidden relative flex-col self-stretch mt-0 w-full max-md:max-w-full  bg-top bg-cover md:bg-cover bg-no-repeat  lg:pt-20"
      style={{
            ...
            <div className="flex  relative flex-col   rounded-[347px] max-md:max-w-full">
              ...
              <div className="relative">
                ...
                <blockquote className="relative self-start lg:mt-16 mt-[75px] font-urbanist tracking-wide para2 text-[#EAEAEA] w-[95%] mx-auto max-md:max-w-full md:text-center">
                  {/* “LightBeam Access Governance is a promising solution for
                  seeing user access within organizations. It offers clear
                  visibility into access permissions across several dozen shared
                  folders used, enabling informed and appropriate access control
                  decisions.” */}
